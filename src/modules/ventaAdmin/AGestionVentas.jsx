...
const AGestionVentas = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchVentas(), fetchProductos(), fetchClientes()])
      .then(() => {
  ...
  const agregarProducto = (producto) => {
    ...
    if (productoExistente) {
      ...
      setProductosSeleccionados([...productosSeleccionados, { ...producto, cantidad: 1, total: producto.precio }])
    }
  }
  ...
  const handleSubirEvidenciaPago = (ventaId) => {
    // Mostramos el loader mientras se procesa la subida de evidencia
    Swal.fire({
    ...
    // Aquí iría la lógica para subir la evidencia de pago
    // Por ahora solo simulamos la acción con un timeout
    setTimeout(() => {
      Swal.close() // Cerramos el loader
    }, 1500)
  }

  const handleSubirEvidenciaEnvio = (ventaId) => {
    // Mostramos el loader mientras se procesa la subida de evidencia
    Swal.fire({
      title: "Subiendo evidencia de envío",
      text: "Por favor espere...",
      allowOutsideClick: false,
      didOpen: () => {
        Swal.showLoading()
    ...
    // Aquí iría la lógica para subir la evidencia de envío
    // Por ahora solo simulamos la acción con un timeout
    setTimeout(() => {
      Swal.close() // Cerramos el loader
    }, 1500)
